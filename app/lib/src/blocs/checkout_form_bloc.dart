...
/*      formData['security'] = checkoutForm.nonce.updateOrderReviewNonce;
  ...
  Future applyWallet(Map data) async {
    final response = await apiProvider.postWithCookies('/wp-admin/admin-ajax.php', data);
    print(response.body);
    if (response.statusCode == 200) {
      //updateOrderReview2();
      return true;
  ...
  void updateOrderReview2() async {
    ...
    formData['wc-ajax'] = 'update_order_review';
    formData['_wp_http_referer'] = '%2Fwoocommerce%2F%3Fwc-ajax%3Dupdate_order_review';
  ...
  Future<dynamic> processPPCPGateway() async {
    var data = new Map<String, dynamic>();
    data['nonce'] = checkoutFormData!.data.ppcSaveCheckoutForm;
    data['form_encoded'] = getQueryString(formData);
    Response response = await apiProvider.postWithCookiesEncoded('/?wc-ajax=ppc-save-checkout-form', data);
    if(response.statusCode == 200) {
      var data = new Map<String, dynamic>();
      data['nonce'] = checkoutFormData!.data.ppcCreateOrder;
      data['form_encoded'] = getQueryString(formData);
      data['bn_code'] = 'Woo_PPCP';
      data['context'] = 'checkout';
      data['createaccount'] = 'false';
      data['funding_source'] = 'paypal';
      data['payment_method'] = 'ppcp-gateway';
      data['order_id'] = '0';
      Response response = await apiProvider.postWithCookiesEncoded('/?wc-ajax=ppc-create-order', data);
