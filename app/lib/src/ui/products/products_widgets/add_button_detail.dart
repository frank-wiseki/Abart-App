...
class _AddButtonDetailState extends State<AddButtonDetail> {
  ...
  Widget build(BuildContext context) {
    if(getQty(widget.product) != 0 || isLoading)
      return Container(
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              isLoading ? SizedBox(
                ...
                width: 20.0,
                child: Text(getQty(widget.product).toString(), textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodyText1!.copyWith(
      ...
      child: ElevatedButton(
        ...
        onPressed: widget.product.stockStatus == 'outofstock' ? null : () async {
          if(widget.product.type == 'variable' || widget.product.type == 'grouped') {
            await _bottomSheet(context);
            setState(() {});
          } else {
...
/*  decreaseQty() async {
  ...
  getQty(Product product) {
    var count = 0;
    if(context.read<ShoppingCart>().cart.cartContents.any((element) => element.productId == product.id)) {
      if(product.type == 'variable') {
        ...
        return count;
      } else return context.read<ShoppingCart>().cart.cartContents.firstWhere((element) => element.productId == product.id).quantity;
    } else return count;
  }
