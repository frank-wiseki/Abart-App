...
class _CheckoutOnePageState extends State<CheckoutOnePage> {
  ...
  _applyWalletBalance(AsyncSnapshot<OrderReviewModel> snapshot) {
    return SliverToBoxAdapter(
      child: snapshot.data!.balance != 0 && snapshot.data!.balance < double.parse(snapshot.data!.totalsUnformatted.total) || widget.checkoutFormBloc.formData['partial_pay_through_wallet'] != null ? Column(
        children: [
          CheckboxListTile(
              controlAffinity: ListTileControlAffinity.leading,
              title: Text('Apply Wallet Balance - ' + parseHtmlString(snapshot.data!.balanceFormatted)),
              value: widget.checkoutFormBloc.formData['partial_pay_through_wallet']  == 'on', onChanged: (value) async {

            if(value != null) {
              setState(() {
                walletChecked = value;
              });
              Map data = {'action': 'woo_wallet_partial_payment_update_session', 'checked': walletChecked ? 'true' : 'false', '_wp_http_referer': '%2Fwoocommerce%2F%3Fwc-ajax%3Dupdate_order_review'};
              widget.checkoutFormBloc.formData['partial_pay_through_wallet'] = walletChecked ? 'on' : 'off';
              widget.checkoutFormBloc.formData['checked'] = walletChecked  ? 'true' : 'false';
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            if(snapshot.data!.cartFees.length > 0)
              Column(
                children: snapshot.data!.cartFees.map((fee) => Column(
                  children: [
                    const SizedBox(height: 6.0),
                    Row(
                      children: <Widget>[
...
print(widget.checkoutFormBloc.formData['payment_method']);
    ...
    if (widget.checkoutFormBloc.formData['payment_method'] == 'ppcp-gateway') {
      //widget.checkoutFormBloc.orderReviewData.paymentMethods

      //
      setState(() {
        isLoading = true;
      });
      var id = await widget.checkoutFormBloc.processPPCPGateway();
      if(id is String) {
        openWebView('https://www.sandbox.paypal.com/checkoutnow?token='+id);
      }
      setState(() {
        isLoading = false;
      });
